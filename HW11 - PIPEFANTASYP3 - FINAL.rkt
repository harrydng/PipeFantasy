...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |HW11 - PIPEFANTASYP3 - FINAL|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
#|
(pipe-fantasy (gamestate-init GRID-DIM 1 1 "right" TILE-SIZE PIPE-WIDTH INIT-GAME-PIPES DOUBLE-PASS-THROUGH-PIPES))

(pipe-fantasy (make-gamestate (place-pipe (make-grid GRID-DIM INIT-GAME-PIPES)
                                          (make-pipe (string=? "right" "top")
                                                     (string=? "right" "bottom")
                                                     (string=? "right" "left")
                                                     (string=? "right" "right"))
                                          1 1)
...
(define INIT-GAME-PIPES (list (list 1 1 (make-pipe #false #false #false #true))
...
;; pipe-can-flow pipe direction -> [Optional direction]
...
;; pipe->image: pipe Number Number boolean direction -> image
...
(define (pipe->image pipe tile-size pipe-width filled? goo-direction)
  (local
       ...
       (local
         [(define mid-left-value (quotient (- tile-size pipe-width) 2))
          ...
          (define pipe-sub-imgs 
            (map (lambda (kind pipe-fn) (kind (if (pipe-fn pipe)
                                                  pipe-color
                                                  "transparent")))
     ...
     (define (place-pipe-on-grid grid-img x y pipe)
       (local
         [(define img-x (floor (* tile-size (+ x 1/2))))
          ...
          (define goo-direction
            (if (my-member? (list x y)
                            (map (lambda (elem) (list (first elem) (second elem)))
                                 (if (and (not (empty? goo-flow))
                                     ...
                                     goo-flow)))
                (if (and (not (empty? goo-flow))
                         (not (false? (first goo-flow))))
                    (second (first goo-flow))
                    #false)
                #false))]
         (place-image
          (pipe->image pipe
                      ...
                      (my-member? (list x y)
                                  (map (lambda (elem) (list (first elem) (second elem)))
                                       (if (and (not (empty? goo-flow))
                                           ...
                                           goo-flow)))
                      goo-direction
                      ;;compare the x and y of goo-flow and the given x and y,
                      ;;then if they match, return the goo-flow direction within the list
                      ;;check if the current pipe is filled twice,
                      ;;;seperate it ... for it to flow twice... 
                      )
          img-x
          img-y
          grid-img)))
...
(define-struct gamestate [grid goo-flow incoming-pipes tile-size pipe-width num-replaced time-left])
;; A gamestate is
...
;; num-replaced: the number of pipes that have been replaced per click.
;; time-left: the time countdown for the goo to flow,
;;            then the maximum time the player can play until all pipes run out

;; get-score : GameState -> Integer
;; computes the current score of the game
(check-expect (get-score (make-gamestate (make-grid 8 empty)
                                         (list (list 1 1 "right"))
                                         DOUBLE-PASS-THROUGH-PIPES
                                         200 300
                                         2 140)) -100)
(check-expect (get-score (make-gamestate (make-grid 8 empty)
                                         (list (list 1 1 "right"))
...
(define (get-score gamestate)
  (* 50 (- (- (length (gamestate-goo-flow gamestate)) 1)
           (gamestate-num-replaced gamestate))))
...
(define (draw-interface gs)
  (local
    [(define incoming-pipes (gamestate-incoming-pipes gs))
     ...
     (define goo-flow (gamestate-goo-flow gs))
     (define incoming-pipes-img
       (above
              (text "NEXT\nPIPE" 20 "grey")
              (foldr (lambda (pipe img) (above (pipe->image pipe tile-size pipe-width #false goo-flow)
                                               img))
                     empty-image
    ...
    (beside (draw-grid gs) (above (text (string-append "Score:" (number->string (get-score gs))) 20 "red")
                                  incoming-pipes-img))))
...
(define (gamestate-place-pipe-on-click gs x y event)
  (local
    [(define incoming-pipes (gamestate-incoming-pipes gs))
     ...
     (define tile-size (gamestate-tile-size gs))
     (define num-replaced (gamestate-num-replaced gs))
     (define grid-x (image-coor->grid-coor x tile-size))
     (define grid-y (image-coor->grid-coor y tile-size))
     (define dim (grid-dim griddy))
     ;;goo-present: GooFlow Integer Integer --> Boolean
     ;; #true if if the x-coord clicked = x-coord of goo, same goes for y-coord within the gooflow
     ;; if the goo stops flowing (#false) then return false
     ; [any-of #false (list Integer Integer direction)]]
     (define (goo-present gooflow x-coord y-coord)
       (ormap (lambda (goo) (if (false? goo)
                                #false
                                (and (= x-coord (first goo))
                                     (= y-coord (second goo)))))
               gooflow))]
    ...
    (cond
      [(and (string=? "button-down" event)
            ...
            (< grid-x dim)
            (< grid-y dim)
            (not (goo-present (gamestate-goo-flow gs) grid-x grid-y)))
...
;; gamestate-init : Integer Integer Integer direction Integer Integer (any-of Integer pipes) (any-of Integer pipes) -> gamestate
...
(define (gamestate-init grid-dim start-x start-y start-dir tile-size pipe-width starting-pipes incoming-pipes)
  (local
    [(define start-pipe 
     ...
     (define start-grid (place-pipe (make-grid grid-dim starting-pipes)
                                    start-pipe
                                    start-x
                                    start-y))]
...
;; decreases 1 every tick until reaches 0, then goo will flow
(check-expect (timer (make-gamestate (make-grid 8 empty)
                                     (list (list 1 1 "right"))
                                     DOUBLE-PASS-THROUGH-PIPES
                                     200 300
                                     2 140))
              (make-gamestate (make-grid 8 empty)
                              (list (list 1 1 "right"))
                              DOUBLE-PASS-THROUGH-PIPES
                              200 300
                              2 139))
...
(define (timer gamestate)
  (cond
    [(> (gamestate-time-left gamestate) 0)
     (make-gamestate (gamestate-grid gamestate)
                     (gamestate-goo-flow gamestate)
                     (gamestate-incoming-pipes gamestate)
                     (gamestate-tile-size gamestate)
                     (gamestate-pipe-width gamestate)
                     (gamestate-num-replaced gamestate)
                     (- (gamestate-time-left gamestate) 1))]
    [(= (gamestate-time-left gamestate) 0)
     (make-gamestate (gamestate-grid gamestate)
                     (grid-goo-propagate
...
;(pipe-fantasy (gamestate-init GRID-DIM 1 1 "right" TILE-SIZE PIPE-WIDTH empty DOUBLE-PASS-THROUGH-PIPES))
;(pipe-fantasy (gamestate-init GRID-DIM 1 1 "right" TILE-SIZE PIPE-WIDTH INIT-GAME-PIPES DOUBLE-PASS-THROUGH-PIPES))